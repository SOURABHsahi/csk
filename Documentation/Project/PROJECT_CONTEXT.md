...
- [x] Phase 86 (LinkedIn-Style Post Card Overhaul & Content Truncation "...see more") — Transformed the post card layout into an authentic LinkedIn-standard experience. Implemented post content truncation (`...see more` / `see less` toggle) for posts exceeding 220 characters or 3+ lines, preserving clickable hashtags and URLs in both preview and expanded states. Replaced squircle avatar with a circular avatar (`w-12 h-12 rounded-full`), formatted author designation and department as a professional headline, and added a timestamp with globe icon. Upgraded interaction counts with overlapping reaction pills and repost counter. Refactored the action bar to 4 classic LinkedIn actions: Like (with hover reaction picker), Comment (speech bubble), Repost (repeat icon with universal share modal), and Send (paper airplane with instant clipboard link copy). Built and deployed to IIS.
